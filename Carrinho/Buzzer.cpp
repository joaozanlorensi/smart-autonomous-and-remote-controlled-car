...
void Buzzer::sing(int song) {
  ...
  if (song == THEME_VICTORY) {
    ...
    for (int thisNote = 0; thisNote < size; thisNote++) {
      ...
      buzz(melodyPin, victorySongMelody[thisNote], noteDuration);
  ...
  } else if (song == THEME_CHALLENGE) {
    int size = sizeof(challengeSongMelody) / sizeof(int);
    for (int thisNote = 0; thisNote < size; thisNote++) {

      // to calculate the note duration, take one second
      // divided by the note type.
      // e.g. quarter note = 1000 / 4, eighth note = 1000/8, etc.
      int noteDuration = 1000 / challengeSongTempo[thisNote];

      buzz(melodyPin, challengeSongMelody[thisNote], noteDuration);
